...
const static std::string kNaviLeftPath = "/sys/class/leds/core_navi_left/brightness";
const static std::string kNaviRightPath = "/sys/class/leds/core_navi_right/brightness";
...
int main() {
    ...
    std::ofstream naviLeft(kNaviLeftPath);
    if (!naviLeft) {
        LOG(ERROR) << "Failed to open " << kNaviLeftPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }

    std::ofstream naviRight(kNaviRightPath);
    if (!naviRight) {
        LOG(ERROR) << "Failed to open " << kNaviRightPath << ", error=" << errno
                   << " (" << strerror(errno) << ")";
        return -errno;
    }
